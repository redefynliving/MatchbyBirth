      ...
      >
          ...
          <div className="absolute inset-0 z-20 bg-background/95 flex flex-col p-6 md:p-8 overflow-y-auto">
            <div className="flex items-center justify-between border-b border-border pb-4 mb-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-foreground">Report sample</h3>
              </div>
              <button
                type="button"
            ...
            <div className="flex-1 space-y-6 max-w-xl mx-auto text-left">
              <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
                ...
                <h4 className="font-extrabold text-foreground mt-2 mb-3">Relationship snapshot</h4>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  The full report turns the free score into a readable pattern for {names[0]} and {names[1]}: what tends to feel easy, where the pacing may catch, and which conversation is most worth having first.
                </p>
                <div className="mt-4 filter blur-[3.5px] select-none text-xs text-muted-foreground leading-relaxed space-y-2">
                  <p>Your detailed snapshot explains the strongest score in plain language, then names the lowest-scoring area without turning it into a verdict.</p>
                  <p>It also includes a concrete next step, so the report feels useful after the first read instead of just interesting.</p>
                </div>
              </div>
              ...
              <div className="bg-card border border-border rounded-2xl p-6 shadow-sm relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent flex flex-col justify-end items-center p-6 text-center pb-8 z-10">
                  ...
                  <p className="text-xs text-muted-foreground max-w-xs mt-1">Unlock communication notes, friction points, timing cues, and practical conversation prompts.</p>
                </div>
                <div className="filter blur-[5px] select-none text-xs text-muted-foreground leading-relaxed space-y-2">
                  ...
                  <p>This section explains what each person may need when a conversation gets tense, including what to say sooner and what to stop assuming.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="p-6 md:p-8">
          ...
          <h2 id="report-modal-title" className="mt-3 pr-8 text-3xl font-semibold tracking-tight">
            Turn the score into something you can actually use
          </h2>
          <p className="mt-2 text-sm font-medium text-foreground">
            {names.join(' + ')}
          </p>
          <p className="mt-3 text-sm leading-6 text-muted-foreground flex flex-wrap items-center gap-x-2 gap-y-1">
            <span>A 9-section report written from this exact result, not a generic sign description.</span>
            <button
              type="button"
          ...
          <div className="mt-6 divide-y divide-border">
            {[
              [MessageCircle, 'Communication pattern', 'What may be easy to say, what may get misread, and how to repair it.'],
              [HeartHandshake, 'Connection strengths', 'The parts of the match most likely to feel supportive or energizing.'],
              [TriangleAlert, 'Watch areas', 'Where different pacing, needs, or expectations may create friction.'],
              [Compass, 'Next conversation prompts', 'Specific questions to use instead of vague compatibility advice.'],
        ...
        <section className="border-t border-border bg-muted/35 p-6 md:border-l md:border-t-0 md:p-8 flex flex-col justify-between">
          <div>
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-card text-primary shadow-sm">
              <LockKeyhole className="h-4 w-4" />
            </span>
            <h3 className="mt-5 text-xl font-semibold text-foreground">What you get for $9.99</h3>
            <p className="mt-2 text-sm leading-6 text-foreground/90">
              A private report page, email delivery, and a PDF you can keep.
            </p>
            ...
            <div className="mt-5 grid gap-2 text-left text-xs leading-5 text-foreground/90">
              {[
                [ClipboardList, '9 personalized sections'],
                [MessageCircle, 'Plain-language friction notes'],
                [Download, 'Private link plus downloadable PDF'],
              ].map(([Icon, label]) => (
                <span key={label} className="flex items-center gap-2 rounded-xl border border-border bg-card/75 px-3 py-2 font-semibold">
