...
export function renderBlogIndexHtml({ template, allPosts = posts } = {}) {
  const cards = allPosts.map(articleCard).join('');
  const body = `
...
export function renderCategoryHtml({ template, category, allPosts = posts } = {}) {
  ...
  const cards = categoryPosts.map(articleCard).join('');
  const body = `
    <main class="static-blog-shell">
      <header>
        <p>Match by Birth Guides</p>
  ...
  if (Array.isArray(post.comparisonRows) && post.comparisonRows.length > 0) {
    sections.push(`
      <section class="static-enhancement">
        ...
        <table>
          <thead>
            <tr><th>Signal</th><th>Best use</th><th>Watch out</th></tr>
          </thead>
          <tbody>
  ...
  return `
    <section class="static-related">
      ...
      ${relatedPosts.map((relatedPost) => `
        <article class="static-post-card">
          <h3><a href="/blog/${escapeHtml(relatedPost.slug)}">${escapeHtml(relatedPost.title)}</a></h3>
...
export function renderArticleHtml({ template, post, allPosts = posts } = {}) {
  const body = `
    <main class="static-blog-shell">
      <article>
  ...
  return renderDocument({
    ...
    description: post.description || stripHtml(post.content).slice(0, 155),
    route: `/blog/${post.slug}`,
    body,
    head: [
      renderJsonLd(buildArticleSchema(post)),
...
export function prerenderBlogHtml({
  ...
  for (const post of allPosts) {
    written.push(writeHtmlFile(
      outputRoot,
      `/blog/${post.slug}`,
      renderArticleHtml({ template, post, allPosts }),
    ));
  }
